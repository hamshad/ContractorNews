    ...
    <div class="col-12 col-md-12 box1">
      <div class="col-12 col-md-12 col-lg-12 demo box1-1 py-3">
        <div class="owl-carousel owl-theme article">
          <?php if (!empty($allActiveCoverStroyArticles)) {
            //echo '<pre>'; print_r($allActiveCoverStroyArticles); die;
            foreach ($allActiveCoverStroyArticles as $cover_story) {
                ...
                <script type="application/ld+json">
                  {
                    ...
                    "@type": "NewsArticle",
                    "headline": "<?php echo str_replace('"', '\"', strip_tags($cover_story->title)); ?>",
                    "datePublished": "<?php echo date('c', strtotime($cover_story->published_date)); ?>",
                    "url": "<?php echo base_url() . $cover_story->id . '/' . urlencode($title_url); ?>",
                    "description": "<?php echo str_replace('"', '\"', strip_tags($cover_story->small_description)); ?>",
                    "image": "<?php echo base_url() . $cover_story->image_url . '/' . $cover_story->image_name; ?>",
                    "publisher": {
                      "@type": "Organization",
                ...
                <script type="application/ld+json">
                  {
                    "@context": "https://schema.org",
                    "@type": "NewsArticle",
                    "headline": "<?php echo str_replace('"', '\"', strip_tags($article->title)); ?>",
                    "datePublished": "<?php echo date('c', strtotime($article->published_date)); ?>",
                    "url": "<?php echo base_url() . $article->id . '/' . urlencode($title_url); ?>",
                    "image": "<?php echo base_url() . $article->image_url . '/' . $article->image_name; ?>",
                    "publisher": {
                      "@type": "Organization",
                      "name": "<?php echo !empty($site_settings->site_name) ? str_replace('"', '\"', strip_tags($site_settings->site_name)) : 'Contractor News'; ?>",
                      "logo": {
                ...
                <tbody class="construct-tbody">
                  ...
                  ?>
                    <tr>
                      ...
                      <td>
                        <p><?php $location = !empty($update->state_short_name) ? $update->state_short_name : "NA";
                            echo $location; ?></p>
                      </td>
          ...
          ?>
            <table class="table table-bordered table-striped construct">
              ...
              <tbody>
                ...
                ?>
                  <tr>
                    <td><?php $location = !empty($event->state_short_name) ? $event->state_short_name : "Not Disclosed";
                        echo $location; ?></td>
...
<script type="application/ld+json">
  {
    ...
    "name": "<?php echo !empty($site_settings->site_name) ? str_replace('"', '\"', strip_tags($site_settings->site_name)) : 'Contractor News'; ?>",
    "url": "<?php echo base_url(); ?>",
    "logo": "<?php echo base_url(); ?>assets/images/Contractor-News-Logo-transparent.PNG",
    "sameAs": [
      "<?php echo !empty($site_settings->facebook_url) ? $site_settings->facebook_url : ''; ?>",
      "<?php echo !empty($site_settings->twitter_url) ? $site_settings->twitter_url : ''; ?>",
      "<?php echo !empty($site_settings->linkedin_url) ? $site_settings->linkedin_url : ''; ?>"
    ]
  }
</script>
